...
//72d2b0e08c5279964fb9b54c715e0fd6
    ...
    if (message.member.user.tag === "Groovy#7254") {
        ...
        voiceChannel.join().then(connection => {
            const dispatcher = connection.play('Audios/gemido.mp3');
    ...
    if (message.content === 'jorgito estas muerto?') {
        ...
        var voiceChannel = message.member.voice.channel;
        voiceChannel.join().then(connection => {
            const dispatcher = connection.play('Audios/fallecio.mp3');
            dispatcher.on("end", end => {voiceChannel.leave()});
        }).catch(err => console.log(err));
        isReady = true;
    }
    ...
    if (message.content === 'jorgito navidad') {
        ...
        var voiceChannel = message.member.voice.channel;
        voiceChannel.join().then(connection => {
            const dispatcher = connection.play('Audios/navidad.mp3');
            dispatcher.on("end", end => {voiceChannel.leave()});
        }).catch(err => console.log(err));
        isReady = true;
    }
    ...
    if (message.content === 'jorgito chupalo') {//https://media.metrolatam.com/2019/11/13/capturadepantall-646a87dedefce6ec690766c86c927d5b-1200x600.jpg
        ...
        var voiceChannel = message.member.voice.channel;
        voiceChannel.join().then(connection => {
            const dispatcher = connection.play('Audios/chupalo.mp3');
            dispatcher.on("end", end => {voiceChannel.leave()});
        }).catch(err => console.log(err));
        isReady = true;
    }
    ...
    if (message.content === 'jorgito te amo') {
        ...
        var voiceChannel = message.member.voice.channel;
        voiceChannel.join().then(connection => {
            const dispatcher = connection.play('Audios/teamo.mp3');
            dispatcher.on("end", end => {voiceChannel.leave()});
        }).catch(err => console.log(err));
        isReady = true;
    }
    ...
    if (message.content === 'jorgito quedate en casa') {
        ...
        var voiceChannel = message.member.voice.channel;
        voiceChannel.join().then(connection => {
            const dispatcher = connection.play('Audios/quedateencasa.mp3');
            dispatcher.on("end", end => {voiceChannel.leave()});
        }).catch(err => console.log(err));
        isReady = true;
    }
